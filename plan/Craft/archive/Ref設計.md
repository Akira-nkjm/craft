...
2. **Python annotation と API ペイロードを 1:1 対応** — `Annotated[T, Ref(...)]` と `{"$ref": ..., "system": ...}` が同じ意味
...
### 3.2 `system` vs `scope` の用語

veriq は `scope=`、本プロジェクトのドメイン用語は `system`。
**veriq の `scope=` をそのまま使う**（noise less than aliasing）。
本プロジェクトの「system」は **scope と等価** と用語集に明記。
...
| キーワード名 | **`scope=`**（veriq に揃える）。`system` は概念用語 |
