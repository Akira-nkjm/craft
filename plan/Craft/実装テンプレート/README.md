...
**方式 C** (Component を base class、`@component` decorator 廃止) + **system 中心レイアウト** での最終形。
...
各 system は以下のファイルを **必要に応じて** 持つ:
- `components.py` — `class X(Component, ...):` の hardware 定義
- `configs.py` — `class X(Config):` の 設定値定義
- `analyses.py` — `@analysis` 関数
- `scope.py` — `vq.Scope` 定義（新 system 時のみ）
- `data.toml` — インスタンスデータ + Config 値

---

## ユーザが触るのは `systems/<name>/` だけ

| やりたいこと | 編集するファイル |
|---|---|
| 新 component (hardware) を追加 | `systems/<sub>/components.py` |
| 新 config (非 hardware) を追加 | `systems/<sub>/configs.py` |
...
| 既存に field 追加 | 同上 |
| 新 system 追加 | `systems/<new>/` を新規作成 |
| 解析関数追加 | `systems/<sub>/analyses.py` |
| veriq scope 設定 | `systems/<sub>/scope.py`（新 system 時のみ） |
...
|---|---|---|---|---|---|
| power | [[実装テンプレート/systems/power/components\|components]] | - | [[実装テンプレート/systems/power/analyses\|analyses]] | [[実装テンプレート/systems/power/scope\|scope]] | [[実装テンプレート/systems/power/data\|data]] |
| thermal | [[実装テンプレート/systems/thermal/components\|components]] | - | - | - | - |
| com | [[実装テンプレート/systems/com/components\|components]] | - | - | - | - |
| cdh | [[実装テンプレート/systems/cdh/components\|components]] (OBC: Singleton default) | - | - | - | - |
| mission | - | [[実装テンプレート/systems/mission/configs\|configs]] (Config の例) | - | - | - |
