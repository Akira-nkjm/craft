...
mirror: subsystems/power/analyses.py
---
...
電力サブシステムの解析関数。
**関数は decorator のまま**（`@analysis`）。subsystem は親ディレクトリから自動推論。
...
from craft.subsystems.power.components import (
    Battery,
    PDM,
    SolarPanel,
)
...
| 書き方 | 結果 |
|---|---|
| `@analysis(...)` (subsystem 引数なし) | **ディレクトリから自動推論** → `"power"` |
| `@analysis(subsystem="thermal")` | 明示 → `"thermal"` |
| `@analysis(subsystem=None)` | **明示で ad-hoc 化**、veriq 非登録 |

→ 通常は引数なしで OK。ad-hoc にしたい時のみ `subsystem=None` を書く。
...
def test_total_power_by_mode():
    from craft.subsystems.power.analyses import total_power_by_mode
    from craft.subsystems.power.components import PDM
