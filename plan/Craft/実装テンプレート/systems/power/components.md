...
mirror: systems/power/components.py
---
...
ファイルパスが `systems/power/components.py` なので、`Component.__init_subclass__` がこれを検出し `system="power"` を自動セット。

明示したい場合は:
```python
class Battery(Component, TemperatureSensitive, system="power_alt"):
    ...
```
PEP 487 のキーワード引数で渡す。
