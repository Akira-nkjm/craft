...
mirror: systems/mission/configs.py
---
...
ファイル `systems/mission/configs.py` から `system="mission"` 自動セット。
...
GET    /api/configs                                 # 全 Config 一覧（全 system）
```
...
- ❌ 同一 system に同名 Config を複数（registry 例外）
