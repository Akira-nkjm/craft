...
```python
class Battery(Component, TemperatureSensitive, system="power_alt", plural="batteries"):
    ...
```

PEP 487 はクラス定義時のキーワード引数を `__init_subclass__` に転送する仕様。
→ `system` と `plural` を **クラス定義の最後にキーワード引数として** 書ける。
...
def _infer_system_from_caller() -> str:
    ...
    while frame:
        ...
        if "systems" in path.parts:
            idx = path.parts.index("systems")
            return path.parts[idx + 1]  # "power"
        frame = frame.f_back
    raise RuntimeError("Cannot infer system; pass explicitly")
```
...
- ❌ system 自動推論を最終 fallback 無しで実装（テスト時にスタック追跡できないと爆発）
