...
| **検索口** | 「power の battery」「全 analysis」「特定 subsystem の全て」を高速検索 |
...
class AnalysisDefinition:
    ...
    subsystem: str | None                       # None = ad-hoc, str = veriq scope に登録
...
class SourceLocation:
    file: str           # "/abs/path/to/schema/subsystems/power.py"
    line: int
    module: str         # "craft.schema.subsystems.power"
```
...
class UnifiedRegistry:
    ...
    def analyses(self, *, subsystem: str | None = None, verify: bool | None = None) -> list[AnalysisDefinition]: ...

    # ───── メタ照会 ─────
    def subsystems(self) -> set[str]:
        """登録されている全 subsystem 名（component / analysis の和集合）"""

    def has_component(self, subsystem: str, name: str) -> bool: ...
    def has_analysis(self, subsystem: str | None, name: str) -> bool: ...
...
def test_my_thing(fresh_registry):
    class Foo(Component, subsystem="test"): ...
...
```python
class Battery(Component, subsystem="power", registry=my_registry): ...
```
...
```python
# experiment/schema/subsystems/__init__.py
import pkgutil
import importlib
...
app start
  ↓
import craft.subsystems            # subsystems/<name>/{components,configs,analyses}.py を一括 import
  ↓
...
- subsystem 跨ぎ参照（例: `@analysis` の引数型に他 subsystem の Spec を使う）が **import 順序** に依存しないよう、 **遅延参照** を許容
  - `Annotated[BatterySpec, ...]` の `BatterySpec` は `from craft.schema.subsystems.power import BatterySpec` で OK（その前に import されている）
  - 完全な遅延が必要なら文字列アノテーション ... ❌ veriq が嫌うので使えない
  - → **subsystems 配下は import 順序を `__init__.py` で固定**するのが安全（alphabetical で十分なはず）
...
`(subsystem, name)` がキー。重複 = `DuplicateRegistration`。

ただし **同じ subsystem 内で同名 component を上書きしたいケース**（テストで monkey patch 等）は明示的 API で:
```python
...
`(subsystem, name)` がキー。`subsystem=None` の場合は `(None, name)`。
ad-hoc 同名は禁止（typo 防止）。

### 7.3 plural の重複

`(subsystem, plural)` も unique でないと TOML 構造が壊れる:
```toml
...
これは **問題にならない**:
- 全プロセスが同じ subsystem ファイルを import → 同じ登録物になる
- registry は read-only（freeze 後）なので不整合が起きる余地が無い

将来 hot reload で subsystem を動的追加したくなったら → 別途設計（Phase 3 以降）。
...
| キー | `(subsystem, name)` (component) / `(subsystem or None, name)` (analysis) |
| 重複検出 | 必須（typo 防止） |
| plural 衝突検出 | 必須 |
