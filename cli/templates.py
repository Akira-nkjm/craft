...
_HARDWARE_COMPONENTS = '''"""{name} subsystem components."""
...
_CONFIG_ONLY = '''"""{name} subsystem configs."""
...
_SCOPE = '''"""{name} subsystem veriq scope。"""
...
def _build_and_attach() -> type:
    root_model = build_subsystem_root_model("{name}", subsystem_data_path("{name}"))
    {name}.root_model()(root_model)
    for adef in default_registry.analyses(subsystem="{name}"):
        if adef.verify:
...
_DATA_TOML = """# {name} subsystem data
...
def create_subsystem(target: Path, *, name: str, kind: str) -> None:
    """target ディレクトリに subsystem 雛形ファイルを生成。"""
    target.mkdir(parents=True, exist_ok=False)
    (target / "__init__.py").write_text(f'"""{name} subsystem package."""\n')

    if kind == "config-only":
        (target / "configs.py").write_text(_CONFIG_ONLY.format(name=name))
        import_line = f"from subsystems.{name} import configs as _configs  # noqa: F401"
    elif kind == "default":
        (target / "components.py").write_text(
            f'"""{name} subsystem components."""\n'
            "\n"
            "# TODO: ここに `class X(Component, ...):` を書く\n"
        )
        import_line = f"from subsystems.{name} import components as _components  # noqa: F401"
    else:  # hardware
