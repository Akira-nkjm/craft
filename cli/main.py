...
def create_cmd(
    ...
    _bootstrap()
    from cli.error_mapping import apply_cli_result
...
def put_cmd(
    ...
    result = replace_component_op(system, component, instance, payload, expected_etag=resolved_etag)
    apply_cli_result(result)
    typer.echo(f"# ETag: {result.etag}")
    _print_json(result.payload)
...
def patch_cmd(
    ...
    result = patch_component_op(system, component, instance, delta, expected_etag=resolved_etag)
    apply_cli_result(result)
    typer.echo(f"# ETag: {result.etag}")
    _print_json(result.payload)
...
def delete_cmd(
    ...
    result = delete_component_op(system, component, instance, expected_etag=resolved_etag)
    apply_cli_result(result)
