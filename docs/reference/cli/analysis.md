...
| `system: null` | ad-hoc analysis。`craft analysis run _ <name>` で実行 |

---

## `craft analysis run <system|_> <name>`

指定した analysis を実行する。

```bash
# system に紐づく analysis
uv run craft analysis run power total_pdm_power_w

# ad-hoc analysis（system=None）は _ を指定
...
| 引数 / オプション | 必須 | 説明 |
|---|---|---|
| `system \| _` | はい | analysis が属する system 名。`system=None` の場合は `_` |
...
!!! note "`_` の使いどころ"
    `@analysis(system=None, ...)` で登録された analysis は veriq に依存せず、
    CLI から `--payload` で引数を渡して単体実行できる。
    複数 system をまたがる計算や、設計初期の試算に便利。
